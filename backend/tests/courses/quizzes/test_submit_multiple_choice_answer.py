...
class SubmitMultipleChoiceAnswerTests(TestCasesWithUserAuth):
    def get_api_endpoint(
        self, course_slug: str, quiz_slug: str, multiple_choice_question_id: UUID
    ) -> str:
    ...
    def test_no_existing_answer_obj_happy_path(self):
        ...
        self.assertFalse(
            db.MultipleChoiceAnswer.objects.filter(
                quiz_submission=quiz_submission, question=multiple_choice_question
            ).exists()
        )
    ...
    def test_answer_index_out_of_bounds_throws_error(self):
        ...
        expected_body = {
            "error": f"Maximum index for multiple choice question is {len(multiple_choice_question.options) - 1}"
        }
