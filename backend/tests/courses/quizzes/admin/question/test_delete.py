...
class DeleteQuestionTests(TestCasesWithUserAuth):
    def get_api_endpoint(
        self, course_slug: str, quiz_slug: str, question_type: str, checkbox_question_id
    ) -> str:
    ...
    def test_delete_coding_question__happy_path(self):
        ...
        response = self.client.delete(
            self.get_api_endpoint(
                mock_quiz.offering.course.slug,
                mock_quiz.slug,
                "coding",
                mock_coding_question.id,
            ),
        )
        ...
        self.assertFalse(
            db.CodingQuestion.objects.filter(id=mock_coding_question.id).exists()
        )
    ...
    def test_delete_multiple_choice_question__happy_path(self):
        ...
        response = self.client.delete(
            self.get_api_endpoint(
                ...
                mock_quiz.slug,
                "multiple_choice",
                mock_multiple_choice_question.id,
            ),
        )
        ...
        self.assertFalse(
            db.MultipleChoiceQuestion.objects.filter(
                id=mock_multiple_choice_question.id
            ).exists()
        )
    ...
    def test_delete_written_response_question__happy_path(self):
        ...
        self.assertFalse(
            db.WrittenResponseQuestion.objects.filter(
                id=mock_coding_question.id
            ).exists()
        )
    ...
    def test_delete_checkbox_question__happy_path(self):
        ...
        self.assertFalse(
            db.CheckboxQuestion.objects.filter(id=mock_coding_question.id).exists()
        )
