...
class RequestToJoinTeamTests(TestCasesWithUserAuth):
    def test_request_to_join_team_happy_path(self):
        ...
        self.assertEqual(
            team_member.membership_type, db.TeamMember.MembershipType.REQUESTED_TO_JOIN
        )
    ...
    def test_request_to_join_team_over_capacity(self):
        offering = create_offering()
        offering_teams_settings = create_offering_teams_settings(
            offering, max_team_size=0
        )
    ...
    def test_request_to_join_team_past_deadline(self):
        offering = create_offering()
        offering_teams_settings = create_offering_teams_settings(
            offering, formation_deadline=datetime.now() - timedelta(days=1)
        )
